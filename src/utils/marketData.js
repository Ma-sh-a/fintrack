...
export async function fetchMarketPrice(ticker, type) {
  ...
  if (type === "Криптовалюта") {
    ...
    if (!id)
      throw new Error(
        `Тикер ${t} не найден в списке поддерживаемых (BTC, ETH, USDT, SOL, TON, BNB, XRP, ADA, DOGE)`,
      );
  ...
  if (!rows || rows.length === 0)
    throw new Error(`Тикер ${t} не найден на MOEX`);
  ...
  if (prev) return prev;
  throw new Error(
    `Котировка для ${t} недоступна — рынок закрыт, и цена закрытия не найдена`,
  );
}
