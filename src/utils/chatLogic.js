...
export function buildAnswer(question, ctx) {
  ...
  if (q.includes('копилк') || q.includes('накоплен')) {
    ...
    const goalMatch = savingGoals.find((g) => q.includes(g.name.toLowerCase()))
    if (goalMatch) {
      const saved = deposits.filter((d) => d.goalId === goalMatch.id).reduce((s, d) => s + Number(d.amount), 0)
  ...
  if (matchedCategory) {
    ...
    if (matchedCategory.monthlyLimit > 0) {
      ...
      if (left < 0) return `На «${matchedCategory.name}» потрачено ${sum.toLocaleString('ru-RU')} ₽ — лимит ${matchedCategory.monthlyLimit.toLocaleString('ru-RU')} ₽ превышен на ${Math.abs(left).toLocaleString('ru-RU')} ₽.`
  ...
  if (q.includes('совет') || q.includes('лимит') || q.includes('бюджет')) {
    const overLimit = categories.find((c) => {
      ...
      const sum = monthTx.filter((t) => t.type === 'expense' && t.categoryId === c.id).reduce((s, t) => s + Number(t.amount), 0)
      return sum > c.monthlyLimit
    })
    if (overLimit) return `Категория «${overLimit.name}» уже превысила лимит на этот месяц — стоит притормозить траты там. Подробности — на дашборде.`
