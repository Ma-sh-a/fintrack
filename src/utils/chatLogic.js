function normalize(word) {
  return word
    .toLowerCase()
    .replace(
      /(ами|ями|ого|его|ой|ей|ый|ий|ая|яя|ое|ее|ых|их|у|ю|а|я|ы|и|е|о)$/,
      "",
    );
}
...
export function buildAnswer(question, ctx) {
  ...
  if (q.includes("сравн") || q.includes("прошл")) {
    ...
    const prevExpense = prevTx
      ...
      .reduce((s, t) => s + Number(t.amount), 0);
    if (prevExpense === 0)
      return "За предыдущий месяц данных нет — не с чем сравнить.";
  ...
  if (q.includes("копилк") || q.includes("накоплен")) {
    ...
    if (savingGoals.length === 0)
      return "Пока нет ни одной копилки — создай на странице «Копилки».";
  ...
  if (q.includes("инвестиц") || q.includes("портфел")) {
    if (investments.length === 0)
      return "Портфель пуст — добавь активы на странице «Инвестиции».";
    const total = investments.reduce(
      (s, a) => s + Number(a.quantity || 0) * Number(a.currentPrice || 0),
      0,
    );
    const cost = investments.reduce(
      (s, a) => s + Number(a.quantity || 0) * Number(a.purchasePrice || 0),
      0,
    );
  ...
  if (matchedCategory) {
    const sum = monthTx
      .filter(
        (t) => t.type === "expense" && t.categoryId === matchedCategory.id,
      )
      .reduce((s, t) => s + Number(t.amount), 0);
  ...
  if (q.includes("совет") || q.includes("лимит") || q.includes("бюджет")) {
    ...
    if (overLimit)
      return `Категория «${overLimit.name}» уже превысила лимит на этот месяц — стоит притормозить траты там. Подробности — на дашборде.`;
