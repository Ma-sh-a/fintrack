...
describe('calcRiskScore', () => {
  ...
  it('портфель только из облигаций — низкий риск', () => {
    const enriched = [enrichAsset({ type: 'Облигации', quantity: 10, purchasePrice: 100, currentPrice: 100 })]
    expect(calcRiskScore(enriched)).toBe(1)
  })

  it('портфель только из криптовалюты — высокий риск', () => {
    const enriched = [enrichAsset({ type: 'Криптовалюта', quantity: 1, purchasePrice: 1000, currentPrice: 1000 })]
...
describe('groupSum', () => {
  it('группирует и суммирует по ключу', () => {
    const items = [{ broker: 'A', value: 100 }, { broker: 'B', value: 50 }, { broker: 'A', value: 30 }]
    const result = groupSum(items, (i) => i.broker, (i) => i.value)
