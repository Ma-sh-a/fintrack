...
const MOCK_NOTES = ['Кофейня', 'Такси', 'Продукты', 'Аптека', 'Подписка', 'Заправка']
  ...
  return (
    <div className="card bank-connect">
      ...
      <p className="muted" style={{ marginBottom: 16 }}>
        Показывает, как выглядела бы автоматическая загрузка операций через Open Banking API.
        Реального соединения с банком нет.
      </p>
      ...
      {step === 'list' && (
        <div className="bank-list">
          {BANKS.map((b) => (
            <button key={b.id} className="bank-item" onClick={() => selectBank(b)}>
      ...
      {step === 'consent' && (
        <div className="bank-consent">
          <p><strong>{bank.name}</strong> запрашивает доступ:</p>
          <ul className="consent-list">
            <li>✓ Просмотр истории операций за 90 дней</li>
            <li>✓ Просмотр баланса счёта</li>
          </ul>
          <div className="form-actions">
            <button className="btn-primary" onClick={handleAllow}>Разрешить доступ</button>
            <button className="btn-secondary" onClick={reset}>Отмена</button>
          </div>
        </div>
      )}

      {step === 'loading' && <p className="empty-state">Загружаем операции из {bank.name}...</p>}

      {step === 'no-categories' && (
        <div>
          <p className="empty-state">Сначала добавь хотя бы одну категорию расходов на странице «Категории».</p>
          <button className="btn-secondary" onClick={reset}>Назад</button>
        </div>
      )}

      {step === 'done' && (
        <div>
          <p>Готово — импортировано {importedCount} операций из {bank.name}. Проверь список на странице «Операции».</p>
          <button className="btn-secondary" onClick={reset}>Подключить ещё один банк</button>
        </div>
      )}
    </div>
  )
