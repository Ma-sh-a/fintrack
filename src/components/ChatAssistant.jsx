...
const SUGGESTIONS = ['Баланс за месяц', 'Сравни с прошлым месяцем', 'Есть превышения лимита?', 'Сколько в копилках?']

export default function ChatAssistant({ transactions, monthTx, categories, goals = [], deposits = [], investments = [], selectedMonth }) {
  const [open, setOpen] = useState(false)
  const [messages, setMessages] = useState([
    { from: 'bot', text: 'Привет! Спроси меня про баланс, расходы, лимиты, копилки или инвестиции.' },
  ])
  const [input, setInput] = useState('')
  const listRef = useRef(null)

  const ctx = { transactions, monthTx, categories, goals, deposits, investments, selectedMonth }
      ...
      {open ? (
        <div className="chat-panel">
          ...
          <div className="chat-messages" ref={listRef}>
            {messages.map((m, i) => (
              <div key={i} className={`chat-bubble chat-${m.from}`}>{m.text}</div>
            ))}
            {messages.length <= 1 && (
