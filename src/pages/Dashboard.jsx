...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const onErr = (label) => (err) => {
      ...
      setError(
        "Не удалось загрузить часть данных. Проверь соединение с интернетом.",
      );
    };
    const unsubs = [
      onSnapshot(
      ...
      onSnapshot(
        query(collection(db, "goalDeposits"), where("userId", "==", user.uid)),
        (snap) =>
    ...
    return categories
      ...
      .map((c) => {
        ...
        const overPct = Math.round(
          ((spent - c.monthlyLimit) / c.monthlyLimit) * 100,
        );
  ...
  const savingsTotal = useMemo(() => {
    const savingGoalIds = goals
      .filter((g) => (g.kind || "saving") === "saving")
      .map((g) => g.id);
    return deposits
      .filter((d) => savingGoalIds.includes(d.goalId))
          ...
          <div className="stat-cards">
            <div className="stat-card">
              ...
              <span className="stat-value">
                {savingsTotal.toLocaleString("ru-RU")} ₽
              </span>
            </div>
            <div className="stat-card">
              <span className="stat-label">В инвестициях</span>
              <span className="stat-value">
                {investmentsTotal.toLocaleString("ru-RU")} ₽
              </span>
            </div>
            <div className="stat-card">
          ...
          <div className="card">
            ...
            {overLimitCats.length === 0 ? (
              <p className="empty-state">
                Лимиты не превышены ни по одной категории.
              </p>
            ) : (
