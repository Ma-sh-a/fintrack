...
export default function Investments() {
  ...
  async function handleAdd(e) {
    ...
    setName(''); setTicker(''); setBroker(''); setQuantity(''); setPurchasePrice('')
    setShowForm(false)
  }
  ...
  async function handleManualPrice(asset, value) {
    if (!value) return
    await updateDoc(doc(db, 'investments', asset.id), { currentPrice: Number(value), lastUpdated: new Date().toISOString() })
  }

  const enriched = useMemo(() => assets.map(enrichAsset), [assets])

  const { totalValue, totalCost, totalPnl, totalPnlPct } = useMemo(() => portfolioTotals(enriched), [enriched])

  const byBroker = useMemo(() => groupSum(enriched, (a) => a.broker, (a) => a.value), [enriched])
  const byType = useMemo(() => groupSum(enriched, (a) => a.type, (a) => a.value), [enriched])
    ...
    <div className="page">
      ...
      <div className="stat-cards" style={{ marginTop: 20 }}>
        ...
        <div className="stat-card">
          ...
          <span className={`stat-value ${totalPnl >= 0 ? 'positive' : 'negative'}`}>
            {totalPnl >= 0 ? '+' : ''}{totalPnl.toLocaleString('ru-RU')} ₽ ({totalPnlPct >= 0 ? '+' : ''}{totalPnlPct.toFixed(1)}%)
          </span>
        </div>
      </div>
      ...
      {showForm && (
        <form className="transaction-form" onSubmit={handleAdd}>
          <input placeholder="Название актива" value={name} onChange={(e) => setName(e.target.value)} />
          <input placeholder="Тикер" value={ticker} onChange={(e) => setTicker(e.target.value)} />
          <select value={type} onChange={(e) => setType(e.target.value)}>
            {ASSET_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <input placeholder="Брокер / банк" value={broker} onChange={(e) => setBroker(e.target.value)} />
          <input type="number" placeholder="Количество" value={quantity} onChange={(e) => setQuantity(e.target.value)} />
          <input type="number" placeholder="Цена покупки за единицу, ₽" value={purchasePrice} onChange={(e) => setPurchasePrice(e.target.value)} />
        ...
        {enriched.map((a) => (
          <li key={a.id} className="asset-item">
            <div className="asset-item-head">
              ...
              <button className="btn-link danger" onClick={() => handleDelete(a.id)}>Удалить</button>
            </div>
            <div className="asset-item-meta muted">{a.type} · {a.broker} · {a.quantity} шт.</div>
            <div className="asset-item-prices">
              ...
              {LIVE_PRICE_TYPES.includes(a.type) && (
                <button className="btn-link" disabled={updatingId === a.id} onClick={() => handleRefreshPrice(a)}>
            ...
            <div className="asset-item-result">
              ...
              <span className={a.pnl >= 0 ? 'positive' : 'negative'}>
                {a.pnl >= 0 ? '+' : ''}{a.pnl.toLocaleString('ru-RU')} ₽ ({a.pnlPct >= 0 ? '+' : ''}{a.pnlPct.toFixed(1)}%)
              </span>
            </div>
          </li>
        ))}
        {enriched.length === 0 && <p className="empty-state">Портфель пуст — добавьте первый актив.</p>}
      </ul>
    </div>
  )
}
