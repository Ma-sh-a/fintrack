...
export default function TransactionDetail() {
  ...
  if (notFound) return <div className="page"><p>Операция не найдена.</p></div>
  ...
  return (
    <div className="page">
      <button className="btn-link" onClick={() => navigate('/transactions')}>← Назад к списку</button>
      <h1>Операция</h1>
      ...
      {editing ? (
        ...
        <div className="transaction-detail-card">
          <p><strong>Тип:</strong> {tx.type === 'income' ? 'Доход' : 'Расход'}</p>
          <p><strong>Сумма:</strong> {Number(tx.amount).toLocaleString('ru-RU')} ₽</p>
          <p><strong>Категория:</strong> {categoryName}</p>
          <p><strong>Дата:</strong> {tx.date}</p>
          {tx.note && <p><strong>Комментарий:</strong> {tx.note}</p>}
          <div className="form-actions">
            <button className="btn-primary" onClick={() => setEditing(true)}>Редактировать</button>
            <button className="btn-secondary danger" onClick={handleDelete}>Удалить</button>
          </div>
        </div>
      )}
    </div>
