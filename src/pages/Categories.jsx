  ...
  return (
    <div className="page">
      ...
      <ul className="category-list">
        {expenseCats.map((c) => (
          <li key={c.id} className="category-item">
            ...
            {c.monthlyLimit > 0 && (
              <span className="category-limit">лимит {c.monthlyLimit.toLocaleString('ru-RU')} ₽/мес</span>
            )}
            <button className="btn-link danger" onClick={() => handleDelete(c.id)}>Удалить</button>
          </li>
        ))}
        {expenseCats.length === 0 && <p className="empty-state">Категорий расходов пока нет.</p>}
      </ul>
      ...
      <ul className="category-list">
        ...
        {incomeCats.length === 0 && <p className="empty-state">Категорий доходов пока нет.</p>}
      </ul>

      <h2 style={{ marginTop: 28 }}>Лимиты бюджета на этот месяц</h2>
      {expenseCats.filter((c) => c.monthlyLimit > 0).length === 0 && (
        <p className="empty-state">Задай лимит категории выше, чтобы следить за бюджетом.</p>
      )}
      {expenseCats.filter((c) => c.monthlyLimit > 0).map((c) => {
        ...
        return (
          <div key={c.id} className="budget-row">
            <div className="budget-label">
              <span>{c.name}</span>
              <span>{spent.toLocaleString('ru-RU')} / {c.monthlyLimit.toLocaleString('ru-RU')} ₽</span>
            </div>
            <div className="budget-bar">
              <div
                className="budget-bar-fill"
                style={{ width: `${pct}%`, background: pct >= 100 ? '#F6C94D' : c.color }}
              />
            </div>
          </div>
        )
